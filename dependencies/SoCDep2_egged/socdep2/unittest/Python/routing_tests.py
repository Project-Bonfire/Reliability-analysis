...
# Add Imports here:
from socdep2.RoutingAlgorithms.Routing import generate_noc_route_graph
from socdep2.RoutingAlgorithms.Routing_Functions import check_deadlock_freeness, degree_of_adaptiveness, \
    extended_degree_of_adaptiveness, return_turn_model_name
from socdep2.ArchGraphUtilities.AG_Functions import generate_ag
...
class RoutingTesting(TestCase):
    ...
    def test_routing_functions(self):
        ...
        for turn_model in PackageFile.routing_alg_list_3d:
            ...
            shmu_4_test.setup_noc_shm(ag_4_test, Config.TurnsHealth, False)
            noc_rg = generate_noc_route_graph(ag_4_test, shmu_4_test, turn_model, False, False)
            self.assertEqual(check_deadlock_freeness(noc_rg), True,msg="Current Ralgo %s"%turn_model)
